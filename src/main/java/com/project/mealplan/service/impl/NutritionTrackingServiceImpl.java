...
public class NutritionTrackingServiceImpl implements NutritionTrackingService {
    ...
    private Map<String, BigDecimal> calculateDailyConsumed(Long userId, LocalDate date) {
        ...
        if (cachedLog.isPresent()) {
            ...
            result.put("calories", log.getTotalCalories() != null ? log.getTotalCalories() : BigDecimal.ZERO);
        ...
        List<FoodLog> foodLogs = foodLogRepository.findByUser_UserIdAndConsumeDateBetweenOrderByConsumeDateAsc(userId,
                startOfDay, endOfDay);
        for (FoodLog foodLog : foodLogs) {
            ...
            Map<String, BigDecimal> nutrition = calculateRecipeNutrition(recipe);
            totalProtein = totalProtein.add(nutrition.getOrDefault("protein", BigDecimal.ZERO).multiply(quantity));
            totalCarbs = totalCarbs.add(nutrition.getOrDefault("carbs", BigDecimal.ZERO).multiply(quantity));
    ...
    public MealConsumedResponse logMealConsumed(Long userId, Long mealSlotId, Boolean consumed) {
        ...
        MealSlot mealSlot = mealSlotRepository.findByIdAndMealDay_MealPlan_User_UserId(mealSlotId, userId)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "Meal slot not found or access denied"));
        ...
        return MealConsumedResponse.builder()
                ...
                .message(isNowConsumed ? "Đã đánh dấu bữa ăn đã được thực hiện" : "Đã hủy đánh dấu bữa ăn")
                .build();
    }
